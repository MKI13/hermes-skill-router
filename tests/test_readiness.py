...
def test_all_command_dependencies_available_is_ready():
    ...
    assert result["dependency_checks"] == [
        ...
        {"type": "command", "name": "gh", "available": True},
    ]


def test_missing_command_is_dependency_missing():
    result = evaluate(
        skill_content("requirements:\n  commands:\n    - git\n    - gh"),
    ...
    assert result["setup_needed"] is False
    assert result["dependency_checks"][1] == {
        "type": "command",
        "name": "gh",
        "available": False,
    }
...
def test_required_skill_missing_is_dependency_missing():
    ...
    assert result["readiness_status"] == DEPENDENCY_MISSING
    assert result["dependency_checks"][0] == {
        "type": "skill",
        "name": "github",
        "available": False,
    }
...
def test_legacy_hermes_prerequisites_are_supported():
    ...
    assert result["readiness_status"] == READY
    assert {check["type"] for check in result["dependency_checks"]} == {"command", "config"}
